...
@interface phoneViewController ()<UIScrollViewDelegate,UIAlertViewDelegate>
{
    int count ;
    UIView *colorView ;
    UIView *lineView;
...
static phoneViewController *editViewController = nil;
...
- (void)viewDidLoad {
    ...
    {
        ALAsset *asset=[self.images objectAtIndex:i];
        ALAssetRepresentation *assetRep = [asset defaultRepresentation];
        CGImageRef imgRef = [assetRep fullResolutionImage];
        UIImage *posterImage = [UIImage imageWithCGImage:imgRef
        ...
        _drawView.backgroundImage = posterImage;
        scrollView.contentSize = CGSizeMake(scrollView.frame.size.width*count, 0);
        count++;
    }
    count--;
...
-(void)addAction:(UIButton *)sender
{
    ...
    _isInViewController = NO ;
    pick.isInViewController = _isInViewController ;
    
    pick.addActionConformsBlock = ^(LocalPhotoViewController *zzz,NSMutableArray *images){
        count++;
        for (int i = 0 ; i < images.count; i++)
        {
            ALAsset *asset=[images objectAtIndex:i];
            ALAssetRepresentation *assetRep = [asset defaultRepresentation];
            CGImageRef imgRef = [assetRep fullResolutionImage];
            UIImage *posterImage = [UIImage imageWithCGImage:imgRef
                                                       scale:assetRep.scale
            ...
            /*-----------DrawView---------------------*/
            _drawView = [[DrawView alloc]initWithFrame:CGRectMake(scrollView.frame.size.width*(count-1), 0, scrollView.frame.size.width, scrollView.frame.size.height)];
            _drawView.tag = 1000+count;
            [scrollView addSubview:_drawView];
            _drawView.backgroundImage = posterImage;
            scrollView.contentSize = CGSizeMake(scrollView.frame.size.width*count, 0);
        ...
        count--;
        [zzz.navigationController popViewControllerAnimated:YES];
        if (images.count != 0)
        {
